...
DEGsPar <- function(data, samplesheet, cells, contrast, test="wilcox", cores, out, norm){    
  ...
  require(Seurat)
  require(magrittr)
  S.obj <- CreateAssayObject(counts = data)
  if(norm){
  ...
  for(i in 1:cores){
    message(i)
    sub.Objs[[i]] <- CreateSeuratObject(counts = S.obj$data[geneList[[i]],])
  }
  end_time <- Sys.time()
  ...
  rm(geneList)
  gc()    
  cl <- makeCluster(cores, type="PSOCK",outfile="deg-log.txt")
  ...
  res <- foreach(tmp.obj=sub.Objs) %dopar% {     
    ...
    require(magrittr)
    message("In parallel...")
    message(dim(tmp.obj@assays$RNA@layers$counts))
